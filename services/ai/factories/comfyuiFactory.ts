...
export class ComfyUIAIGenerationFactory implements AIGenerationFactory {
  ...
  async generateImagePrompts(_config: AppConfig, _story: Story, _numberOfPrompts: number): Promise<string[]> {
    throw new Error('ComfyUI provider does not support image prompt generation. Use Gemini provider for text.');
  }
  ...
  async generateAudio(_config: AppConfig, _story: Story): Promise<GeneratedAudio> {
    throw new Error('ComfyUI provider does not support audio generation. Use Gemini, Chatterbox, or Kokoro provider for audio.');
  }
