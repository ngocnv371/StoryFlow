...
const ProjectIdeasGenerator: React.FC = () => {
  ...
  return (
    <>
      <button
        onClick={handleOpen}
        className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all shadow-lg bg-slate-900 text-indigo-600 border border-indigo-700/40 hover:bg-indigo-900/30"
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold text-slate-100">Project Idea Generator</h2>
                <p className="text-sm text-slate-500">Generate 10 new project ideas from a theme using text generation.</p>
              </div>
              <button onClick={handleClose} disabled={isBusy} className="p-2 hover:bg-slate-800 rounded-full transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
            ...
            <div className="p-6 overflow-y-auto space-y-5">
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500"
                    />
                  </div>
                </>
              )}
              ...
              {step === 'generating' && (
                <div className="py-10 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-100">Generating 10 ideas...</h3>
              ...
              {step === 'review' && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold text-slate-100">Approve ideas</h3>
                    <span className="text-xs font-bold uppercase tracking-wider bg-indigo-900/30 text-indigo-600 px-3 py-1 rounded-full">
                  ...
                  <div className="space-y-2 max-h-[48vh] overflow-y-auto pr-1">
                    {ideas.map((idea, index) => (
                      <label key={`${index}-${idea}`} className="flex items-start gap-3 p-3 rounded-xl border border-slate-700 hover:border-indigo-700/40 hover:bg-indigo-900/40 transition-colors cursor-pointer">
                        <input
                          type="checkbox"
                          checked={approved[index] || false}
                          onChange={() => toggleApproved(index)}
                          className="mt-1 h-4 w-4 rounded border-slate-700 text-indigo-600 focus:ring-indigo-500"
                        />
              ...
              {step === 'creating' && (
                <div className="py-10 flex flex-col items-center justify-center text-center space-y-4">
                  <div className="w-10 h-10 border-4 border-indigo-700/40 border-t-indigo-600 rounded-full animate-spin"></div>
                  <div>
                    <h3 className="text-lg font-bold text-slate-100">Creating projects...</h3>
            ...
            <div className="p-6 bg-slate-900 border-t flex justify-end gap-3">
              <button
                onClick={handleClose}
                disabled={isBusy}
                className="px-5 py-2.5 rounded-lg border border-slate-700 text-slate-200 font-medium hover:bg-slate-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Cancel
              </button>
